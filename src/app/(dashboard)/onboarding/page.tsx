...
import { UserPlus, Plus, Loader2, Search } from "lucide-react"
  ...
  useEffect(() => {
    fetch("/api/onboarding")
      ...
      .then(d => {
        ...
        setData(items.filter((r: any) => r.type === "onboarding"))
        setLoading(false)
      })
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="h-11 w-11 rounded-xl bg-emerald-500 flex items-center justify-center text-white">
            <UserPlus className="h-5 w-5" />
          </div>
          <div>
            <h1 className="text-2xl font-bold">Onboarding</h1>
            <p className="text-sm text-muted-foreground">Neue Mitarbeiter einrichten</p>
          </div>
        </div>
        <Link href="/onboarding/new?type=onboarding">
          <Button className="bg-emerald-600 hover:bg-emerald-700"><Plus className="h-4 w-4 mr-1.5" /> Neues Onboarding</Button>
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-muted-foreground">
          {data.length === 0 ? "Noch keine Onboardings vorhanden." : "Keine Ergebnisse gefunden."}
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/30">
                ...
                <th className="text-left px-4 py-3 font-medium text-muted-foreground">Startdatum</th>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground">Status</th>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground">Checkliste</th>
            ...
            <tbody>
              {filtered.map((item: any) => {
                ...
                const checklistDone = item.checklist?.filter((c: any) => c.done).length || 0
                const checklistTotal = item.checklist?.length || 0
                return (
                  <tr
                    key={item.id}
                    className="border-b last:border-0 hover:bg-muted/30 cursor-pointer transition-colors"
                    onClick={() => router.push(`/onboarding/${item.id}`)}
                  >
