...
export async function PUT(req: NextRequest, { params }: Ctx) {
  ...
  await query(
    "UPDATE order_products SET name=?, description=?, category_id=?, inventory_item_id=?, supplier_id=?, emoji=?, price_estimate=?, sort_order=?, requires_description=? WHERE id=?",
...
export async function DELETE(_req: NextRequest, { params }: Ctx) {
  ...
  if (!session?.role.includes("admin")) return NextResponse.json({ error: "Forbidden" }, { status: 403 })
  const { id } = await params
  await query("UPDATE order_products SET active = 0 WHERE id = ?", [id])
  return NextResponse.json({ success: true })
}
